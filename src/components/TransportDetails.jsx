...
export default function TransportDetails() {
  ...
  useEffect(() => {
    const fetchTransportDetails = async () => {
      ...
      try {
        ...
        const fetchedTransport = await res?.data?.data.data;
        ...
        setError(err?.response?.data?.message || err?.message || "Failed to fetch transport details.");
        toast.error(err?.response?.data?.message || err?.message || "Failed to fetch transport details.");
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) return <p className="text-center mt-4">Loading transport details...</p>;
      ...
      transport ? (
        <main className="transport-detail-main-container container py-5">
            <div className="car-details-container">
              <Link to="/transports" className="back-link text-decoration-none d-inline-flex align-items-center mb-4 text-primary fw-bold">
                <i className="bi bi-arrow-left me-2"></i>
                Back to {type} transports
              </Link>
              <img src={transport.imageUrl || 'https://via.placeholder.com/800x500'} alt={`${transport.brand} ${transport.model} image`} className="car-detail-image"/>
              <div className="mt-4">
                <h1 className="fw-bold mb-2 text-primary">{transport.brand} {transport.model}</h1>
                <p className="text-muted fs-5">Category: {transport.category}</p>
                <p className="text-muted fs-5">Capacity: {transport.seatingCapacity} passengers</p>
                <p className="text-success fs-4">Price per day: ${transport.pricePerDay}</p>
              </div>
