  ...
  carData.forEach((car) => {
    Object.entries(car).map(([key, value]) => {
      ...
      if (
        ...
        (key === "brand" || key === "model" || key === "category")
      ) {
    ...
    <header className="transport-header">
      ...
      <form className="search-form">
        <button
          ...
          onClick={(e) => handleInputClick(query, setSearchParams, e)}
        >
        ...
        <input
          ...
          onChange={(e) => handleInputChange(e, setQuery, setSearchParams)}
        />
      </form>
    </header>
  );
...
function FilterBtn({
  ...
  return (
    <Link
      ...
      to={link}
      className={`filter-btn ${isRating ? "rating" : ""} ${
        isActive ? "active" : ""
      }`}
    >
      {isRating ? (
    ...
    >
      <div>
        ...
        <div className="car-rating">
          ...
          <span>
            {car.averageRating || car.rating} ({car.reviewCount || 0})
          </span>
        </div>
...
function CarNotFound() {
  return (
    <div className="car-not-found-container">
      ...
      <p>
        We couldn't find any vehicles that meet all your current criteria. Try
        broadening your search by adjusting your filters, selecting more models,
...
function Pagination({
  ...
  const pageEl = typeFilter.length
    ? Math.ceil(filteredCars.length / 9)
    : Math.ceil(transports.length / 9);
  const PageNO = [];
  ...
  return (
    <div className="pagination-container">
      <button
        disabled={
          displayPage[0] === 0 ||
          (typeFilter.length && filteredCars.length <= 9)
        }
      ...
      <div className="pagination-numbers">{pageNoEl}</div>
      <button
        disabled={
          transports.length < displayPage[1] ||
          (typeFilter.length && filteredCars.length <= 9)
        }
        onClick={() => {
...
export default function Transports() {
  ...
  const typeFilter = allFilters.filter(
    (filter) => searchParams.get(filter) !== null
  );
  const filterValues = typeFilter.map((filter) => ({
      ...
      try {
        ...
        const normalized = (
          ...
          imageUrl:
            t.image ||
            t.imageUrl ||
            "https://via.placeholder.com/300x200?text=Transport",
  ...
  if (typeFilter.length) {
    filterValues.forEach((filter) => {
      ...
      for (let i = 0; i < transports.length; i++) {
        // Use transports instead of carData
        const car = transports[i]; // Use transports instead of carData
        if (key === "rating") {
          if (
            String(Math.round(car[key])) === value &&
            !filteredCars.includes(transports[i])
          ) {
            filteredCars.push(car);
          }
        } else if (key === "pricePerDay") {
          if (
            Math.round(car[key]) < Number(value) &&
            !filteredCars.includes(transports[i])
          ) {
            filteredCars.push(car);
          }
    ...
    filteredCars = filteredCars.filter(
      ({
        ...
        return (
          (filters.fuelType === null || filters.fuelType === fuelType) &&
          (filters.seatingCapacity === null ||
            filters.seatingCapacity === String(seatingCapacity)) &&
          (filters.transmission === null ||
            filters.transmission === transmission) &&
          (filters.pickupLocation === null ||
            filters.pickupLocation === pickupLocation) &&
          (filters.pricePerDay === null ||
            Number(filters.pricePerDay) > pricePerDay) &&
          (filters.rating === null ||
            filters.rating === String(Math.round(rating)))
        );
      }
    );
  ...
  const carCardEl = displayedCars.length ? (
      ...
      <main className="transport-main">
        {isFilterAvailable ? (
          ...
          <Filter
            ...
            setIsFilterAvailable={setIsFilterAvailable}
          />
        )}
        <section>
          <Header
            ...
            isAdmin={isAdmin}
          />
          <div className="car-list-container">{carCardEl}</div>
          <Pagination
